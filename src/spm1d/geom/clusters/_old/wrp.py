...
class _Cluster(object):
	...
	iswrapped   = False   # True only for boundary clusters AND circular domains
...
class WrappedCluster(object):
	iswrapped = True
	other     = None   # wrapped cluster (right boundary)
...
class ClusterList(list):
	...
	def wrap(self):
		if len(self)>1:
			c0,c1  = self[0], self[-1]
			if c0.isLboundary and c1.isRboundary and (c0.sign==c1.sign):
