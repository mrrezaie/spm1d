...
class SPM04TestCase(object):
	def __init__(self, dataset, testname, infkwargs, alpha=0.05):
		...
		spmi           = self.get_spm()
		if isinstance(spmi, list):
			self.spmi  = [SPM04(x)  for x in spmi]
		else:
			self.spmi      = SPM04( spmi )
	...
	def get_spm(self):
		...
		spmi  = fn(*self.args).inference(self.alpha, **self.infkwargs)
		return spmi
